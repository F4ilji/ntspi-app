 ...
 *
 * OPTIMIZATION: Добавлен мониторинг памяти для предотвращения утечек.
 */
class ConnectToImapTask
{
    ...
    public function run(?string $accountName = null): Client
    {
        ...
        try {
            // OPTIMIZATION: Логируем память до подключения
            $this->logMemoryUsage('before_imap_connection', [
                'account' => $accountName,
            ]);
