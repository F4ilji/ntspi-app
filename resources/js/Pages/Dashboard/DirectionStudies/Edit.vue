  ...
  <div class="min-h-screen bg-background-2">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
        <div class="flex items-center h-full gap-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-lg font-medium text-foreground">Редактирование направления подготовки</h1>
              <p class="text-xs text-muted-foreground-1">{{ direction?.name }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <!-- Info Content Builder -->
        <div class="bg-layer border border-layer-line rounded-lg shadow-xs">
          <div class="p-6 border-b border-line-2">
            <div class="flex items-center gap-2">
              <DashboardIcon name="document-text" size="5" class="text-primary" />
              <h2 class="text-base font-medium text-foreground">Информация для направления</h2>
            </div>
            <p class="text-xs text-muted-foreground-1 mt-1">
              Создайте содержимое направления используя конструктор
            </p>
          </div>
...
export default {
  ...
  data() {
    return {
      form: {
        ...
        uuid: this.direction?.uuid || '',
        slug: this.direction?.slug || '',
        code: this.direction?.code || '',
        lvl_edu: this.direction?.lvl_edu || '',
        info: this.direction?.info || []
      },
      errors: {},
      processing: false
    }
  ...
  mounted() {
    this.SET_DOCUMENT_TITLE(`Редактирование направления - ${this.direction?.name}`);
  },
  ...
  methods: {
    ...
    submit() {
      ...
      this.$inertia.put(route('dashboard.direction-studies.update', this.direction.id), this.form, {
        onFinish: () => {
