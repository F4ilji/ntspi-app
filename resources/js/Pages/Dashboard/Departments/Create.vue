<template>
  <DashboardLayout>
    ...
    <template #header-title>Создание кафедры</template>
    <template #header-subtitle>Добавление новой кафедры в институт</template>
      ...
      <form @submit.prevent="submit">
        ...
        <div class="px-6 py-4 bg-surface/50 border-t border-layer-line flex items-center justify-end gap-3">
          ...
          >
            ...
            {{ processing ? 'Создание...' : 'Создать кафедру' }}
          </button>
        </div>
      </form>
    </div>
...
export default {
  ...
  mounted() {
    this.SET_DOCUMENT_TITLE('Создание кафедры');
  },
  ...
  methods: {
    ...
    submit() {
      ...
      this.$inertia.post(route('dashboard.departments.store'), this.form, {
        onFinish: () => {
