      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
        <div class="flex items-center h-full gap-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-lg font-medium text-foreground">Редактирование группы</h1>
              <p class="text-xs text-muted-foreground-1">{{ group.title }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-layer border border-layer-line rounded-lg shadow-xs">
        <div class="p-6 border-b border-line-2">
          ...
          <p class="text-xs text-muted-foreground-1 mt-1">
            Измените данные учебной группы
          </p>
        </div>
...
export default {
  ...
  data() {
    return {
      form: {
        ...
        faculty_id: this.group?.faculty_id || '',
        education_form_id: this.group?.education_form_id || ''
      },
      errors: {},
      processing: false
    }
  ...
  mounted() {
    this.SET_DOCUMENT_TITLE(`Редактирование группы - ${this.group?.title}`);
  },
  ...
  methods: {
    submit() {
      ...
      this.$inertia.put(route('dashboard.educational-groups.update', this.group.id), this.form, {
        onFinish: () => {
