      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
        <div class="flex items-center h-full gap-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-lg font-medium text-foreground">Редактирование программы</h1>
              <p class="text-xs text-muted-foreground-1">{{ program?.name }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="bg-layer border border-layer-line rounded-lg shadow-xs">
          ...
          <div class="p-6">
            <div class="space-y-6">
              <div>
                <label for="name" class="block text-sm font-medium text-foreground mb-2">Название программы <span class="text-rose-500">*</span></label>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label for="lvl_edu" class="block text-sm font-medium text-foreground mb-2">Уровень образования <span class="text-rose-500">*</span></label>
                ...
                <div>
                  <label for="status" class="block text-sm font-medium text-foreground mb-2">Статус программы <span class="text-rose-500">*</span></label>
              ...
              <div>
                <label for="lang_stud" class="block text-sm font-medium text-foreground mb-2">Язык обучения <span class="text-rose-500">*</span></label>
              ...
              <div>
                <label for="direction_study_id" class="block text-sm font-medium text-foreground mb-2">Направление подготовки</label>
        ...
        <div class="bg-layer border border-layer-line rounded-lg shadow-xs">
          <div class="p-6 border-b border-line-2">
            <div class="flex items-center gap-2">
              ...
              <h2 class="text-base font-medium text-foreground">Описание программы</h2>
            </div>
          </div>
          <div class="p-6">
            <ContentBuilder v-model="form.about_program" label="О программе" />
          </div>
        </div>
        ...
        <div class="bg-layer border border-layer-line rounded-lg shadow-xs">
          <div class="p-6 border-b border-line-2">
            <div class="flex items-center gap-2">
              ...
              <h2 class="text-base font-medium text-foreground">Особенности программы</h2>
            </div>
          </div>
          <div class="p-6">
            <ContentBuilder v-model="form.program_features" label="Особенности" />
          </div>
        </div>
...
export default {
  name: 'EducationalProgramEdit',
  components: { DashboardIcon, FlashMessages, ContentBuilder },
  props: {
  ...
  data() {
    return {
      form: {
        ...
        lvl_edu: this.program?.lvl_edu || '',
        status: this.program?.status || '',
        lang_stud: this.program?.lang_stud || '',
        direction_study_id: this.program?.direction_study_id || '',
        about_program: this.program?.about_program || [],
        program_features: this.program?.program_features || []
      },
      errors: {},
      processing: false
    }
  },
  mounted() { this.SET_DOCUMENT_TITLE(`Редактирование программы - ${this.program?.name}`); },
  methods: {
    getShortStatusLabel(label) { return label ? label.split('.')[0] : '—'; },
    submit() {
      this.processing = true;
      this.errors = {};
      this.$inertia.put(route('dashboard.educational-programs.update', this.program.id), this.form, {
