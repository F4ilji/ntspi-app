			...
			<div class="grow">
				<div class="grid sm:flex sm:justify-between sm:items-center gap-2">
					<BreadcrumbsWrapper v-if="breadcrumbs">
						...
						<li v-if="breadcrumbs.mainSection" class="text-sm">
							<span class="flex items-center text-gray-500 hover:text-primary cursor-pointer" @click.prevent="handleSectionClick(breadcrumbs.mainSection)">
						...
						<li v-if="breadcrumbs.subSection" class="text-sm">
							<span class="flex items-center text-gray-500 hover:text-primary cursor-pointer" @click.prevent="handleSubSectionClick(breadcrumbs.mainSection, breadcrumbs.subSection)">
								<svg class="flex-shrink-0 mx-2 overflow-visible h-2.5 w-2.5 text-gray-400"
										 width="16" height="16"
										 viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
									<path d="M5 1L10.6869 7.16086C10.8637 7.35239 10.8637 7.64761 10.6869 7.83914L5 14"
                ...
                {{ textLimit('Научные журналы', 30) }}
					...
					<BreadcrumbsWrapper v-else>
						<li class="text-sm">
							<Link :href="route('index')" class="flex items-center text-gray-500 hover:text-primary-light" href="/">
								<BasicIcon class="size-5" name="home" />
							</Link>
						</li>
						<li class="text-sm">
